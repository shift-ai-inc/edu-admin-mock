import { Survey, SurveyVersion, Question, QuestionVersion, VersionStatus, QuestionType, QuestionDifficulty, SkillLevel } from '@/types/survey';
...
const generateMockSurveyQuestions = (count: number, surveyId: string, versionNumber: number): QuestionVersion[] => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    questions.push({
      ...
      questionData: {
        ...
        type: (['multiple-choice', 'single-choice', 'free-text'] as QuestionType[])[i % 3],
        options: (i % 3 !== 2) ? [`選択肢A`, `選択肢B`, `選択肢C`, `選択肢D`] : undefined,
...
let mockSurveyDetailsData: MockSurveyDetailsData = mockSurveys.reduce((acc, survey) => {
  const versions: SurveyVersion[] = [
    {
      id: `${survey.id}-v2`,
      surveyId: survey.id,
      versionNumber: 2,
      description: '最新版サーベイ。いくつかの設問を更新し、表現を改善しました。',
      questions: generateMockSurveyQuestions(survey.questionsCount, survey.id, 2),
      questionCount: survey.questionsCount,
      createdAt: new Date(survey.updatedAt.getTime() - (2 * 24 * 60 * 60 * 1000)), // 2 days before survey update
      createdBy: 'admin-user-02',
      status: 'published',
      publishedAt: new Date(survey.updatedAt.getTime() - (1 * 24 * 60 * 60 * 1000)), // 1 day before survey update
    },
    {
      id: `${survey.id}-v1`,
      surveyId: survey.id,
      ...
      questions: generateMockSurveyQuestions(survey.questionsCount - (survey.questionsCount > 2 ? 2:0) , survey.id, 1), // Fewer questions for v1
      questionCount: survey.questionsCount - (survey.questionsCount > 2 ? 2:0),
      createdAt: new Date(survey.createdAt.getTime() - (5 * 24 * 60 * 60 * 1000)), // 5 days before survey creation
      createdBy: 'admin-user-01',
      status: 'archived',
      publishedAt: new Date(survey.createdAt.getTime() - (4 * 24 * 60 * 60 * 1000)),
    },
  ];
  acc[survey.id] = { base: survey, versions: versions.sort((a,b) => b.versionNumber - a.versionNumber) };
  return acc;
}, {} as MockSurveyDetailsData);


export const getSurveyWithVersions = (surveyId: string): (Survey & { versions: SurveyVersion[] }) | null => {
...
export const getSurveyVersionDetails = (surveyId: string, versionId: string): SurveyVersion | null => {
  const detail = mockSurveyDetailsData[surveyId];
  if (!detail) return null;
...
export const addMockSurveyVersion = (surveyId: string, description: string): SurveyVersion | null => {
  const surveyDetail = mockSurveyDetailsData[surveyId];
  if (!surveyDetail) return null;

  const latestVersion = surveyDetail.versions.sort((a,b) => b.versionNumber - a.versionNumber)[0];
  ...
  const newVersion: SurveyVersion = {
    ...
    questions: latestVersion ? JSON.parse(JSON.stringify(latestVersion.questions)) : [], // Deep copy questions from latest version or start fresh
  ...
  updateMockSurvey(surveyId, { 
    updatedAt: new Date(),
    targetSkillLevel: ["entry", "junior", "mid-level"] as SkillLevel[]
  });
  
  return newVersion;
};

export const getQuestionVersionDetails = (questionVersionId: string): QuestionVersion | null => {
  for (const surveyId in mockSurveyDetailsData) {
    const surveyDetail = mockSurveyDetailsData[surveyId];
    for (const surveyVersion of surveyDetail.versions) {
      const foundQuestion = surveyVersion.questions.find(q => q.id === questionVersionId);
      if (foundQuestion) {
...
export const updateMockQuestionVersionContent = (
  ...
  for (const surveyId in mockSurveyDetailsData) {
    const surveyDetail = mockSurveyDetailsData[surveyId];
    for (const surveyVersion of surveyDetail.versions) {
      const questionIndex = surveyVersion.questions.findIndex(q => q.id === questionVersionId);
      if (questionIndex !== -1) {
        ...
        const updatedQuestionData: Question = {
          ...
          text: newText,
          options: newOptions !== undefined ? newOptions : originalQuestionVersion.questionData.options,
          correctAnswer: newCorrectAnswer !== undefined ? newCorrectAnswer : originalQuestionVersion.questionData.correctAnswer,
        };
