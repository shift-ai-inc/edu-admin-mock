...
export default function Login({ onLoginSuccess }: LoginProps) {
  ...
  const [isLoading, setIsLoading] = useState(false); // Kept for potential use in other forms
  const [error, setError] = useState(""); // Kept for potential use in other forms
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Directly call onLoginSuccess and navigate to dashboard
    onLoginSuccess();
    navigate("/");
  };
  ...
  const handlePasswordResetModalSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (resetModalEmail) { // Basic check if email field is not empty
            ...
            <Card>
              ...
              <CardContent>
                <form onSubmit={handleLogin}>
                  {/* Error display is kept but won't be triggered by handleLogin */}
                  {error && (
                  ...
                  <div className="grid gap-4">
                    ...
                    <Button
                      type="submit"
                      className="w-full"
                      // isLoading state for login button is no longer directly managed here
                      // but kept for consistency if other forms use it.
                    >
                      ログイン
                    </Button>
                  </div>
