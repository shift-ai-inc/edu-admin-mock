...
export default function AddAssessmentQuestion() {
  ...
  const { assessmentId, versionId } = useParams<{
    assessmentId: string;
    versionId: string;
  }>();
  ...
  function onSubmit(data: AddQuestionFormValues) {
    ...
    const questionData: Partial<AssessmentQuestionFormData> & {
      assessmentId: string;
      versionId: string;
    } = {
      assessmentId,
    ...
    if (data.type === "single-choice" || data.type === "multiple-choice") {
      ...
      if (data.type === "single-choice") {
        ...
        questionData.correctAnswer =
          data.options?.[parseInt(data.correctAnswerSingle || "-1", 10)]?.text; // Store text for simplicity now
      } else {
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              {(questionType === "single-choice" ||
                ...
                <div className="space-y-4 rounded-md border p-4">
                  ...
                  <FormDescription>
                    {questionType === "single-choice"
                      ? "選択肢を入力し、正解を1つ選択してください。"
                      : "選択肢を入力し、正解を1つ以上選択してください。"}
                  </FormDescription>
                  ...
                  {questionType === "multiple-choice" && fields.length > 0 && (
                    <FormField
                      ...
                      render={() => (
                        // Note: We use form.setValue inside Checkbox onChange
