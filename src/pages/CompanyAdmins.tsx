...
const columns: ColumnDef<Admin>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const isActive = admin.status === "アクティブ";

      const handleToggleStatus = () => alert(`ユーザー ${admin.name} のステータスを ${isActive ? '非アクティブ' : 'アクティブ'} に変更しますか？ (実装が必要です)`);
      const handleChangeRole = () => alert(`ユーザー ${admin.name} の権限を変更しますか？ (実装が必要です)`);
      const handleResetPassword = () => alert(`ユーザー ${admin.name} のパスワードをリセットしますか？ (実装が必要です)`);
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem onClick={handleToggleStatus}>
              {isActive ? (
                <><UserX className="mr-2 h-4 w-4" /> 無効化</>
              ) : (
                <><UserCheck className="mr-2 h-4 w-4" /> 有効化</>
              )}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={handleChangeRole}>
              <Edit className="mr-2 h-4 w-4" /> 権限変更
            </DropdownMenuItem>
